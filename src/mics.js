...
function mountDevices(stdout) {
  const devices = stdout
    .split(/  . /)
    .slice(1)
    .filter((name) => !name.includes("Monitor of "));
...
async function listMics() {
  const { stdout } = await exec(
    "pacmd list-sources | grep -e 'index:' -e device.description -e 'name:' -e 'properties:'"
  );
...
async function listSpeakers() {
  const { stdout } = await exec(
    "pacmd list-sinks | grep -e 'index:' -e device.description -e 'name:' -e 'properties:'"
  );
  return mountDevices(stdout);
}
